...
def save_experiment_report(
    ...
    if valid_results:
        ...
        misclassified_list = [
            r["match_result"]["misclassified"] for r in valid_results
        ]
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        if benign_map is not None:
            _write_map_section(
                f, benign_map, args, section_num, "Benign mAP (Model Predictions as GT)"
            )
            section_num += 1

        if gt_map is not None:
            _write_map_section(
                f, gt_map, args, section_num, "GT mAP (COCO Annotations as GT)"
            )
            section_num += 1

        _write_efficiency_section(f, valid_results, queries_list, elapsed_time, section_num)
        section_num += 1
...
def _write_aggregate_section(
    ...
    bbox_asr = (
        (total_disappeared_all + total_misclassified_all) / total_bboxes_all
        if total_bboxes_all > 0
        else 0.0
    )
    f.write("\n  --- BBox-Level Summary ---\n")
...
def _write_map_section(f, map_result, args, section_num, title):
    ...
    drop_pct = (
        map_result["mAP_drop"] / map_result["orig_mAP"] * 100
        if map_result["orig_mAP"] > 0
        else 0.0
    )
...
def _write_efficiency_section(f, valid_results, queries_list, elapsed_time, section_num):
